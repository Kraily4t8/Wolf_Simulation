...
# loop through prey list to find 5 closest sheep to wolf then take average distance
...
for i in range(len(preyArr[2])):
    ...
    for j in range(len(preyArr[2])):  
        dist = 0        
        dist = distance(preyArr[0][i].get("x"), preyArr[0][i].get("y"), 
                    wolfArr[0][j].get("x"), wolfArr[0][j].get("y"))
